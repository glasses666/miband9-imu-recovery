...
let smooth={ax:null,ay:null,az:null},prevRaw=null,vibrationUntil=0,lastMotion=0;
...
function vecDelta(a,b){if(!a||!b)return 0;return Math.sqrt((a.ax-b.ax)**2+(a.ay-b.ay)**2+(a.az-b.az)**2)}
function consumeSample(now){if(sampleQ.length>24){sampleQ=sampleQ.slice(-10); lastStep=now-10} if(!cur&&sampleQ.length) cur=sampleQ.shift(); while(sampleQ.length && now-lastStep>=10){cur=sampleQ.shift(); lastStep+=10} if(now-lastStep>60) lastStep=now; if(cur){let cal=(state&&state.calibration)||{}, gb=cal.gyro_bias||{}, gate=cal.vibration_gate||{}; let raw={ax:cur.ax||0,ay:cur.ay||0,az:cur.az||0}; let gx=(cur.gx||0)-Number(gb.x||0), gy=(cur.gy||0)-Number(gb.y||0), gz=(cur.gz||0)-Number(gb.z||0); let accelDelta=vecDelta(raw,prevRaw), gyroAbs=Math.sqrt(gx*gx+gy*gy+gz*gz); let vib=(accelDelta>Number(gate.accel_delta_threshold||0.28))||(gyroAbs>Number(gate.gyro_abs_threshold||0.045)); if(vib)vibrationUntil=now+Number(gate.settle_ms||350); let gated=now<vibrationUntil; let alpha=gated ? .08 : ((gyroAbs<.018&&accelDelta<.12) ? .22 : .34); for(const k of ['ax','ay','az']) smooth[k]=smooth[k]===null?raw[k]:smooth[k]*(1-alpha)+raw[k]*alpha; cur.axf=smooth.ax; cur.ayf=smooth.ay; cur.azf=smooth.az; cur.gxc=gx; cur.gyc=gy; cur.gzc=gz; cur.accel_delta=accelDelta; cur.gyro_abs=gyroAbs; cur.vibration_gate=gated; lastMotion=clamp(accelDelta*3+gyroAbs*8,0,1); let targetPitch=Math.atan2(-smooth.ax,Math.sqrt(smooth.ay*smooth.ay+smooth.az*smooth.az))-Number(cal.pitch_rad||0); let targetRoll=Math.atan2(smooth.ay,smooth.az)-Number(cal.roll_rad||0); let poseAlpha=gated ? .05 : .28; pose.pitch=pose.pitch*(1-poseAlpha)+targetPitch*poseAlpha; pose.roll=pose.roll*(1-poseAlpha)+targetRoll*poseAlpha; pose.yaw+=gz*0.010; prevRaw=raw; sampleHist.push({...cur,motion:lastMotion}); if(sampleHist.length>360) sampleHist=sampleHist.slice(-360);}}
