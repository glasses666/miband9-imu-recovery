...
def parse_gb_sport_xms_command_log(log_text: str, command: str, terminal_messages: set) -> dict:
    terminal = {}
    built = {}
    send_requested = {}
    spp_packets = []
    for candidate in parse_structured_app_logs(log_text):
        if candidate.get("command") != command:
            continue
        message = candidate.get("message")
        if message == "payload_built":
            built = candidate
        elif message == "protobuf_send_requested":
            send_requested = candidate
        elif message == "spp_packet":
            spp_packets.append({
                "packet_index": parse_int_token(candidate.get("packet_index"), len(spp_packets) + 1),
                "channel": candidate.get("channel", ""),
                "payload_length": parse_int_token(candidate.get("payload_length"), 0),
                "payload_hex": candidate.get("payload_hex", ""),
                "command_type": parse_int_token(candidate.get("command_type"), -1),
                "command_subtype": parse_int_token(candidate.get("command_subtype"), -1),
            })
        elif message in terminal_messages:
            terminal = candidate
    source = terminal or send_requested or built
    return {
        ...
        "capture_ms": parse_int_token(source.get("capture_ms"), 0),
        "sport_type": parse_int_token(source.get("sport_type"), 0),
        "sport_state": parse_int_token(source.get("sport_state"), 0),
        "proto_sport_state": parse_int_token(source.get("proto_sport_state"), -1),
        "timezone": parse_int_token(source.get("timezone"), 0),
        "payload_bytes": parse_int_token(source.get("payload_bytes"), 0),
        "payload_hex": built.get("payload_hex", source.get("payload_hex", "")),
        "send_requested": bool(send_requested),
        "spp_packets": parse_int_token(source.get("spp_packets"), len(spp_packets)),
        "protobuf_packets": parse_int_token(source.get("protobuf_packets"), 0),
        "activity_packets": parse_int_token(source.get("activity_packets"), 0),
        "other_spp_packets": parse_int_token(source.get("other_spp_packets"), 0),
        "xms_response_8_26_packets": parse_int_token(source.get("xms_response_8_26_packets"), 0),
        "xms_status_8_50_packets": parse_int_token(source.get("xms_status_8_50_packets"), 0),
        "xms_sensor_8_53_packets": parse_int_token(source.get("xms_sensor_8_53_packets"), 0),
        "first_spp_channel": source.get("first_spp_channel", ""),
        "first_spp_hex": source.get("first_spp_hex", ""),
        "last_spp_channel": source.get("last_spp_channel", ""),
        "last_spp_hex": source.get("last_spp_hex", ""),
        "packet_logs": spp_packets,
        "reason": source.get("reason", ""),
    }


def parse_gb_sport_xms_open_log(log_text: str) -> dict:
    return parse_gb_sport_xms_command_log(
        log_text,
        "gb-sport-xms-open",
        {"opener_complete", "opener_failed", "unknown_command"},
    )


def parse_gb_sport_xms_stop_log(log_text: str) -> dict:
    return parse_gb_sport_xms_command_log(
        log_text,
        "gb-sport-xms-stop",
        {"stop_complete", "stop_failed", "unknown_command"},
    )


def resolve_optional_out_dir(args, default_leaf: str) -> Optional[Path]:
    raw = getattr(args, "out_dir", None)
    if not raw:
        return None
    out_dir = Path(raw).expanduser()
    if not out_dir.is_absolute():
        out_dir = repo_root_from_args(getattr(args, "repo_root", None)) / out_dir
    if out_dir.name == "AUTO":
        out_dir = out_dir.parent / default_leaf
    return out_dir
...
def write_jsonl_artifact(path: Path, rows: list[dict]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def save_gb_sport_xms_artifacts(result: Result, parsed: dict, out_dir: Path, label: str) -> dict:
    packet_logs = list(parsed.get("packet_logs") or [])
    xms53_packets = [
        packet for packet in packet_logs
        if parse_int_token(packet.get("command_type"), -1) == 8
        and parse_int_token(packet.get("command_subtype"), -1) == 53
        and packet.get("payload_hex")
    ]
    files = {
        "result": str(out_dir / f"{label}_result.json"),
        "matching_app_log": str(out_dir / "matching_app_log.log"),
        "spp_packets": str(out_dir / "spp_packets.jsonl"),
        "xms53_payloads": str(out_dir / "xms53_payloads.jsonl"),
        "manifest": str(out_dir / "manifest.json"),
    }
    out_dir.mkdir(parents=True, exist_ok=True)
    write_json_artifact(Path(files["result"]), result.to_dict())
    Path(files["matching_app_log"]).write_text(str(result.data.get("matching_app_log", "")), encoding="utf-8")
    write_jsonl_artifact(Path(files["spp_packets"]), packet_logs)
    write_jsonl_artifact(Path(files["xms53_payloads"]), xms53_packets)
    manifest = {
        "command": result.command,
        "ok": result.ok,
        "label": label,
        "packet_logs": len(packet_logs),
        "xms53_payload_packets": len(xms53_packets),
        "xms53_payload_lengths": sorted({parse_int_token(packet.get("payload_length"), 0) for packet in xms53_packets}),
        "files": files,
    }
    write_json_artifact(Path(files["manifest"]), manifest)
    return manifest
...
def parse_find_band_log(log_text: str, command: str = "find-band") -> dict:
    terminal = {}
    started = {}
    stopped = {}
    for candidate in parse_structured_app_logs(log_text):
    ...
    if "did_present" in source:
        parsed["did_present"] = parse_bool_token(str(source.get("did_present", "")))
    if "device_connected" in source:
        parsed["device_connected"] = parse_int_token(source.get("device_connected"), -1)
    return parsed
...
def cmd_band_connect(args) -> Result:
    force_connection_type = (getattr(args, "force_connection_type", "") or "").upper().replace("-", "_")
    result = cmd_app_command(
        args,
        "connect",
        extras={
            ...
            "force_connection_type": force_connection_type,
        },
...
def cmd_band_mi_find_band(args) -> Result:
    duration_ms = max(250, min(int(args.duration_ms), 10000))
    result = cmd_app_command(
        args,
        "mi-find-band",
    ...
    probe = parse_find_band_log(result.data.get("matching_app_log", ""), command="mi-find-band")
    result.command = "band mi-find-band"
    result.data["probe"] = probe
    if result.ok and probe.get("message") != "find_complete":
        result.ok = False
        result.error = probe.get("message") or "mi_find_band_not_completed"
    return result
...
def cmd_band_gb_sport_xms_open(args) -> Result:
    capture_ms = max(500, min(int(args.capture_ms), 60000))
    timezone = "" if args.timezone is None else str(args.timezone)
    result = cmd_app_command(
        args,
        "gb-sport-xms-open",
        extras={
            ...
            "xms_sport_type": str(args.sport_type),
            "xms_sport_state": "1",
            "xms_timezone": timezone,
        },
        wait_timeout=max(args.timeout, (capture_ms // 1000) + 15),
        terminal_messages={"opener_complete", "opener_failed", "unknown_command"},
    )
    opener = parse_gb_sport_xms_open_log(result.data.get("matching_app_log", ""))
    result.command = "band gb-sport-xms-open"
    result.data["opener"] = opener
    if result.ok and opener.get("message") != "opener_complete":
        result.ok = False
        result.error = opener.get("message") or "gb_sport_xms_open_not_completed"
    out_dir = resolve_optional_out_dir(args, f"gb_sport_xms_open_{time.strftime('%Y%m%d_%H%M%S')}")
    if out_dir is not None:
        result.data["artifact_dir"] = str(out_dir)
        result.data["artifact_manifest"] = save_gb_sport_xms_artifacts(result, opener, out_dir, "open")
    return result


def cmd_band_gb_sport_xms_stop(args) -> Result:
    capture_ms = max(500, min(int(args.capture_ms), 60000))
    timezone = "" if args.timezone is None else str(args.timezone)
    result = cmd_app_command(
        args,
        ...
        wait_timeout=max(args.timeout, (capture_ms // 1000) + 15),
        terminal_messages={"stop_complete", "stop_failed", "unknown_command"},
    )
    stopper = parse_gb_sport_xms_stop_log(result.data.get("matching_app_log", ""))
    result.command = "band gb-sport-xms-stop"
    result.data["stopper"] = stopper
    if result.ok and stopper.get("message") != "stop_complete":
        result.ok = False
        result.error = stopper.get("message") or "gb_sport_xms_stop_not_completed"
    out_dir = resolve_optional_out_dir(args, f"gb_sport_xms_stop_{time.strftime('%Y%m%d_%H%M%S')}")
    if out_dir is not None:
        result.data["artifact_dir"] = str(out_dir)
        result.data["artifact_manifest"] = save_gb_sport_xms_artifacts(result, stopper, out_dir, "stop")
    return result


def clone_args_with(**kwargs):
    values = dict(kwargs.pop("_base"))
    values.update(kwargs)
    return argparse.Namespace(**values)


def resolve_required_out_dir(args, default_leaf: str) -> Path:
    raw = getattr(args, "out_dir", "") or str(Path("/tmp") / default_leaf)
    out_dir = Path(raw).expanduser()
    if not out_dir.is_absolute():
        out_dir = repo_root_from_args(getattr(args, "repo_root", None)) / out_dir
    return out_dir
...
def cmd_band_gb_sport_xms_start_stop(args) -> Result:
    base_dir = resolve_required_out_dir(args, f"miband9_gb_sport_xms_start_stop_{time.strftime('%Y%m%d_%H%M%S')}")
    base_values = vars(args)

    open_args = clone_args_with(_base=base_values, capture_ms=args.capture_ms, out_dir=str(base_dir / "open"))
    open_result = cmd_band_gb_sport_xms_open(open_args)

    stop_args = clone_args_with(_base=base_values, capture_ms=args.stop_capture_ms, out_dir=str(base_dir / "stop"))
    stop_result = cmd_band_gb_sport_xms_stop(stop_args)

    if args.stop_verify_settle_ms > 0:
        time.sleep(args.stop_verify_settle_ms / 1000.0)

    stop_verify_args = clone_args_with(_base=base_values, capture_ms=args.verify_capture_ms, out_dir=str(base_dir / "stop_verify"))
    stop_verify_result = cmd_band_gb_sport_xms_stop(stop_verify_args)

    decode_summary = {}
    decode_stdout = ""
    decode_stderr = ""
    decode_returncode = -1
    xms53_path = base_dir / "open" / "xms53_payloads.jsonl"
    decoder_path = repo_root_from_args(args.repo_root) / "tools" / "miband9ctl" / "decode_xms53_packets.py"
    if xms53_path.exists() and decoder_path.exists():
        decode_cmd = [
            sys.executable,
            str(decoder_path),
            str(xms53_path),
            "--out-jsonl",
            str(base_dir / "open" / "decoded_samples.jsonl"),
            "--out-csv",
            str(base_dir / "open" / "decoded_samples.csv"),
            "--summary",
            str(base_dir / "open" / "decode_summary.json"),
        ]
        decoded = subprocess.run(decode_cmd, capture_output=True, text=True, timeout=max(30, args.timeout))
        decode_returncode = decoded.returncode
        decode_stdout = decoded.stdout[-2000:]
        decode_stderr = decoded.stderr[-2000:]
        summary_path = base_dir / "open" / "decode_summary.json"
        if summary_path.exists():
            decode_summary = json.loads(summary_path.read_text(encoding="utf-8"))

    open_manifest = open_result.data.get("artifact_manifest", {})
    stop_manifest = stop_result.data.get("artifact_manifest", {})
    stop_verify_manifest = stop_verify_result.data.get("artifact_manifest", {})
    open_xms53 = int(open_manifest.get("xms53_payload_packets", 0) or 0)
    stop_verify_xms53 = int(stop_verify_manifest.get("xms53_payload_packets", 0) or 0)
    sample_rows = int(decode_summary.get("sample_rows", 0) or 0)
    ok = bool(
        open_result.ok
        and stop_result.ok
        and stop_verify_result.ok
        and open_xms53 > 0
        and stop_verify_xms53 == 0
        and decode_returncode == 0
        and sample_rows > 0
    )
    return Result(
        ok=ok,
        command="band gb-sport-xms-start-stop",
        data={
            "artifact_dir": str(base_dir),
            "open_manifest": open_manifest,
            "stop_manifest": stop_manifest,
            "stop_verify_manifest": stop_verify_manifest,
            "decode_summary": decode_summary,
            "decode_returncode": decode_returncode,
            "decode_stdout_tail": decode_stdout,
            "decode_stderr_tail": decode_stderr,
            "pass_conditions": {
                "open_xms53_payload_packets_gt_0": open_xms53 > 0,
                "stop_verify_xms53_payload_packets_eq_0": stop_verify_xms53 == 0,
                "decoded_sample_rows_gt_0": sample_rows > 0,
            },
            "notes": [
                "The first stop window may include in-flight 8/53 packets before the stop response; stop_verify is the quiescent post-stop proof window."
            ],
        },
        error=None if ok else "gb_sport_xms_start_stop_failed",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    band_connect.add_argument("--force-connection-type", choices=["BLE", "BT_CLASSIC", "BOTH", "ble", "bt_classic", "bt-classic", "both"], default="")
    ...
    band_mi_find_band = band_sub.add_parser("mi-find-band")
    band_mi_find_band.add_argument("--did", default="", help="Optional Mi Fitness device id; redacted from command summaries. Omit to resolve through SportXms device info.")
    band_mi_find_band.add_argument("--duration-ms", type=int, default=3000)
    ...
    band_gb_sport_xms_open = band_sub.add_parser("gb-sport-xms-open")
    band_gb_sport_xms_open.add_argument("--address", default="")
    band_gb_sport_xms_open.add_argument("--capture-ms", type=int, default=5000)
    band_gb_sport_xms_open.add_argument("--sport-type", type=int, default=812)
    band_gb_sport_xms_open.add_argument("--timezone", type=int, default=None, help="Optional Mi Fitness 15-minute timezone slot; omit for Android local timezone")
    band_gb_sport_xms_open.add_argument("--out-dir", default="", help="Optional local artifact directory for full SPP / 8-53 payload logs")
    band_gb_sport_xms_stop = band_sub.add_parser("gb-sport-xms-stop")
    band_gb_sport_xms_stop.add_argument("--address", default="")
    band_gb_sport_xms_stop.add_argument("--capture-ms", type=int, default=5000)
    band_gb_sport_xms_stop.add_argument("--sport-type", type=int, default=812)
    band_gb_sport_xms_stop.add_argument("--timezone", type=int, default=None, help="Optional Mi Fitness 15-minute timezone slot; omit for Android local timezone")
    band_gb_sport_xms_stop.add_argument("--out-dir", default="", help="Optional local artifact directory for full SPP / 8-53 payload logs")
    band_gb_sport_xms_start_stop = band_sub.add_parser("gb-sport-xms-start-stop")
    band_gb_sport_xms_start_stop.add_argument("--address", default="")
    band_gb_sport_xms_start_stop.add_argument("--capture-ms", type=int, default=10000)
    band_gb_sport_xms_start_stop.add_argument("--stop-capture-ms", type=int, default=4000)
    band_gb_sport_xms_start_stop.add_argument("--verify-capture-ms", type=int, default=2500)
    band_gb_sport_xms_start_stop.add_argument("--stop-verify-settle-ms", type=int, default=750)
    band_gb_sport_xms_start_stop.add_argument("--sport-type", type=int, default=812)
    band_gb_sport_xms_start_stop.add_argument("--timezone", type=int, default=None, help="Optional Mi Fitness 15-minute timezone slot; omit for Android local timezone")
    band_gb_sport_xms_start_stop.add_argument("--out-dir", default="", help="Local artifact directory for start/stop/verify/decode bundle; defaults to /tmp")
...
def main(argv=None) -> int:
    ...
    elif args.cmd == "band" and args.band_cmd == "mi-find-band":
        result = cmd_band_mi_find_band(args)
    ...
    elif args.cmd == "band" and args.band_cmd == "gb-sport-xms-open":
        result = cmd_band_gb_sport_xms_open(args)
    elif args.cmd == "band" and args.band_cmd == "gb-sport-xms-stop":
        result = cmd_band_gb_sport_xms_stop(args)
    elif args.cmd == "band" and args.band_cmd == "gb-sport-xms-start-stop":
        result = cmd_band_gb_sport_xms_start_stop(args)
