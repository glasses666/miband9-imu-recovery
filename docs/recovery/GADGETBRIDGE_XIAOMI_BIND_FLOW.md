...
- Android OS Bluetooth state: hand band is currently `BOND_BONDED` as `Xiaomi Smart Band 9 808F` at `08:16:D5:B7:80:8F`.
...
2. `state import --from original-gadgetbridge --address 08:16:D5:B7:80:8F --json`
   - copy only current device DB row and device-specific prefs into hfimucli
   - backup hfimucli DB/prefs first
3. `band bind --address 08:16:D5:B7:80:8F --json`
