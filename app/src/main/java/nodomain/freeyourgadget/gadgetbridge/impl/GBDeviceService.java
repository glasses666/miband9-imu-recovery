...
public class GBDeviceService implements DeviceService {
    ...
    public void onDebugSendRawBytes(final byte[] bytes) {
        Intent intent = createIntent().setAction(ACTION_DEBUG_SEND_RAW_BYTES)
                .putExtra(EXTRA_RAW_BYTES, bytes);
        invokeService(intent);
    }

    @Override
    public void onDebugSendRawProtobufCommand(final byte[] bytes) {
        Intent intent = createIntent().setAction(ACTION_DEBUG_SEND_RAW_PROTOBUF_COMMAND)
                .putExtra(EXTRA_RAW_BYTES, bytes);
        invokeService(intent);
    }
