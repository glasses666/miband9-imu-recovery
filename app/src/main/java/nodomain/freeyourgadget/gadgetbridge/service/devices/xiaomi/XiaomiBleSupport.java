...
public class XiaomiBleSupport extends XiaomiConnectionSupport {
    ...
    private static final int DEFAULT_SAR_WRITE_SIZE = 244;
    ...
    private BluetoothGattCharacteristic characteristicSarNotify;
    private BluetoothGattCharacteristic characteristicSarWrite;
    private int sarWriteSize = DEFAULT_SAR_WRITE_SIZE;
    private final AtomicInteger sarPacketSequenceCounter = new AtomicInteger(0);
    ...
    final AbstractBTLEDeviceSupport commsSupport = new AbstractBTLEDeviceSupport(LOG) {
        ...
        protected TransactionBuilder initializeDevice(final TransactionBuilder builder) {
            ...
            BluetoothGattCharacteristic btCharacteristicSarNotify = null;
            BluetoothGattCharacteristic btCharacteristicSarWrite = null;
            ...
            for (Map.Entry<UUID, XiaomiUuids.XiaomiBleUuidSet> xiaomiUuid : XiaomiUuids.BLE_UUIDS.entrySet()) {
                ...
                if ((currentChar = currentUuidSet.getCharacteristicSarNotify()) == null ||
                        (btCharacteristicSarNotify = getCharacteristic(currentChar)) == null) {
                    LOG.info("SAR notify characteristic is not available for Xiaomi service {}", xiaomiUuid.getKey());
                }

                if ((currentChar = currentUuidSet.getCharacteristicSarWrite()) == null ||
                        (btCharacteristicSarWrite = getCharacteristic(currentChar)) == null) {
                    LOG.info("SAR write characteristic is not available for Xiaomi service {}", xiaomiUuid.getKey());
                }
            ...
            characteristicSarNotify = btCharacteristicSarNotify;
            characteristicSarWrite = btCharacteristicSarWrite;
            sarWriteSize = DEFAULT_SAR_WRITE_SIZE;
            ...
            if (characteristicSarNotify != null) {
                builder.notify(characteristicSarNotify, true);
            }
        ...
        public boolean onCharacteristicChanged(final BluetoothGatt gatt, final BluetoothGattCharacteristic characteristic) {
            ...
            if (characteristicCommandRead.getCharacteristicUUID().equals(characteristicUUID)) {
                ...
                return true;
            } else if (characteristicSarNotify != null && characteristicSarNotify.getUuid().equals(characteristicUUID)) {
                LOG.info("Received Xiaomi SAR V2 notify bytes: {}", GB.hexdump(value, 0, Math.min(value.length, 96)));
                return true;
            }
        ...
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
            ...
            sarWriteSize = Math.max(20, mtu - 3);
        }
    ...
    public void onAuthSuccess() {
        ...
        sarPacketSequenceCounter.set(0);
    }
    ...
    @Override
    public void sendRawProtobufCommandBytes(final String taskName, final byte[] bytes,
                                           @Nullable final XiaomiCharacteristic.SendCallback callback) {
        if (this.characteristicSarWrite == null) {
            LOG.warn("SAR write characteristic is null; cannot send {} protobuf-channel bytes", bytes == null ? 0 : bytes.length);
            if (callback != null) {
                callback.onNack();
            }
            return;
        ...
        if (bytes == null || bytes.length == 0) {
            LOG.warn("Ignoring empty Xiaomi SAR V2 protobuf payload for task '{}'", taskName);
            if (callback != null) {
                callback.onNack();
            }
            return;
        }
        if (!mXiaomiSupport.getAuthService().isEncryptionInitialized()) {
            LOG.warn("Xiaomi SAR V2 protobuf send requested before encryption was initialized");
            if (callback != null) {
                callback.onNack();
            }
            return;
        ...
        final byte[] packet = XiaomiSppPacketV2.newDataPacketBuilder()
                .setChannel(XiaomiChannelHandler.Channel.ProtobufCommand)
                .setSequenceNumber(sarPacketSequenceCounter.getAndIncrement())
                .setOpCode(XiaomiSppPacketV2.DataPacket.getOpCodeForChannel(XiaomiChannelHandler.Channel.ProtobufCommand))
                .setPayload(bytes)
                .build()
                .encode(mXiaomiSupport.getAuthService());
        final int chunkSize = Math.max(20, sarWriteSize);
        final TransactionBuilder builder = commsSupport.createTransactionBuilder("send SAR V2 " + taskName);
        for (int offset = 0; offset < packet.length; offset += chunkSize) {
            builder.write(characteristicSarWrite, Arrays.copyOfRange(packet, offset, Math.min(packet.length, offset + chunkSize)));
        }
        builder.queue(commsSupport.getQueue());
        LOG.info("Queued Xiaomi SAR V2 protobuf payload for '{}': payload={} packet={} chunks={} chunkSize={}",
                taskName,
                bytes.length,
                packet.length,
                (packet.length + chunkSize - 1) / chunkSize,
                chunkSize);
