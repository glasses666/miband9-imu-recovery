...
public final class BtBRQueue {
    ...
    public BtBRQueue(BluetoothAdapter btAdapter, GBDevice gbDevice, Context context, SocketCallback socketCallback, UUID supportedService, int bufferSize) {
        this(btAdapter, gbDevice, context, socketCallback, supportedService, bufferSize, -1);
    }

    public BtBRQueue(BluetoothAdapter btAdapter, GBDevice gbDevice, Context context, SocketCallback socketCallback, UUID supportedService, int bufferSize, int port) {
    ...
    public boolean connect() {
        ...
        try {
            ...
            if (mPort > 0) {
                LOG.info("Creating RFCOMM socket to direct port: {}", mPort);
                try {
                    Method m = btDevice.getClass().getMethod("createRfcommSocket", Integer.TYPE);
                    mBtSocket = (BluetoothSocket) m.invoke(btDevice, mPort);
                } catch (Exception ex) {
                    LOG.error("Reflection failed to create RFCOMM socket for port {}, falling back to UUID: ", mPort, ex);
