# Mi Band 9 HF IMU recovery fork

This public fork documents and prototypes a Xiaomi Smart Band 9 / Mi Band 9 NFC high-frequency IMU path.

Current verified result: Mi Fitness SportXms / Just Dance `sportType=812` can provide official `WearSensorData` accel + gyro callbacks at 100 sample/s when the Band 9 is selected and connected in Mi Fitness. The branch also includes a side-by-side `hfimucli` Android agent, `miband9ctl` CLI tooling, and a local live dashboard.

Start with:

- [`README.HF_IMU_RECOVERY.md`](README.HF_IMU_RECOVERY.md)
- [`docs/recovery/PUBLIC_RESEARCH_CHAIN_20260530.md`](docs/recovery/PUBLIC_RESEARCH_CHAIN_20260530.md)
- [`docs/recovery/RECOVERY_STATE.md`](docs/recovery/RECOVERY_STATE.md)

Public safety note: this fork intentionally excludes auth keys, private app databases, exact device/account identifiers, signed URLs, APKs, firmware blobs, keystores, and local raw artifacts.

---

The upstream Gadgetbridge README continues below for license/project context.
